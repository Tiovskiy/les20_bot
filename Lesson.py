...
# функция ошибки "сжатия"
async def fail(update, context):
    await update.message.reply_text('Вам необходимо сжать фото при отправке')
...
async def detection(update, context):
    ...
    test_dict['conf'] = 0.5 #0.1 0.5 0.99    # порог распознавания
    test_dict['iou'] = 0.5 #0.1 0.5 0.99     # порог искл. объектов
...
def main():
    ...
    application.add_handler(MessageHandler(~filters.PHOTO, fail, block=False)) # символ ~ обзначает "кроме"
